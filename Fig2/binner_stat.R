...
profile <- read.table("./6CAMI.dataset.7binner.running.info.rename.txt",header=T, sep = "\t",check.name = F)
...
pdf(".//realtime_M.pdf",width = 8,height = 4)
realtime
dev.off()
...
pdf("./peak_rss_G.png",width = 8,height = 4)
peak
dev.off()


plot$logPeak <- log(plot$peak_rss_G)
peak <- ggboxplot(plot, y = "logPeak", x = "task",alpha=0.7,fill = "Group",outlier.size = 0.7,
                  #palette = "jco",
                  ggtheme = theme_bw() +
                    theme(axis.text.x = element_text(color="black",size=10,angle=0,hjust= 0.5 ,vjust = 0 ,face="bold"),
                          axis.text.y = element_text(color="black",size=10,face="bold"),
...
pdf("./logPeak.pdf",width = 8,height = 4)
peak
dev.off()
