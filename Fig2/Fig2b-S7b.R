...
profile$binner_list <- sapply(strsplit(profile$binner, "_"), function(x) {
...
dd <- ggbarplot(df, x = "binner_list", y = "mean_counts", fill = "BinScore",size = 0.2,
            ggtheme = theme_bw() +
            ...
            legend = "top",title = "",xlab = 'Binners Combination', ylab = 'Avg (#Bins)',width =0.7)+ 
...
dper <- ggbarplot(df, x = "binner_list", y = "Percentage", fill = "BinScore",size = 0.2,
            ggtheme = theme_bw() +
              theme(axis.text.x = element_text(color="black",size=8,angle=-90,hjust= 0 ,vjust = 0.5 ,face="bold"),
                    axis.text.y = element_text(color="black",size=8,face="bold"),
                    axis.title.y=element_text(color="black",size=8,face="bold"),
                    ...
                    plot.margin = margin(l =5 , r = 20),
                    panel.background = element_blank()),
            legend = "top",title = "",xlab = 'Binners Combination', ylab = 'Percentage',width =0.7)+ 
    scale_fill_manual(values=type_col,labels=F1_list,limits = F1_list, breaks = F1_list)

drawwidth <-  length(levels(df$binner_list))*0.2 + 1
drawheight <- max(nchar(levels(df$binner_list)))*0.1 + 2.5

pdf("FigS7b.pdf",width = 14,height = drawheight)
dper
dev.off()
...
for ( i in dataset_list ) {
  ...
  sub_dper <- ggbarplot(sub_df, x = "binner_list", y = "Percentage", fill = "BinScore",size = 0.2,
              ggtheme = theme_bw() +
                theme(axis.text.x = element_text(color="black",size=8,angle=-90,hjust= 0 ,vjust = 0.5 ,face="bold"),
                      axis.text.y = element_text(color="black",size=8,face="bold"),
                      axis.title.y=element_text(color="black",size=8,face="bold"),
  ...
  drawheight <- max(nchar(levels(sub_df$binner_list)))*0.1 +0.5

pdf(paste("FigS7c.",i,".pdf",sep=""),width = 14,height = drawheight)
print(sub_dper)
dev.off()
